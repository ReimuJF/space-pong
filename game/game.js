...
function create() {
	...
	playerShip = createShip(game.world.centerX - 50, 550, 'Ship');
	computerShip = createShip(game.world.centerX, 50, 'Ship2');
	...
	//pauseButton.onDown.add(fpause, this);
	game.input.onDown.add(fpause, this);
...
function fpause() {
	...
	if (game.paused) {
		textPause = game.add.text(game.world.centerX - 90, game.world.centerY, 'Game Paused', { fill: '#efefef' });
	} else {
		textPause.destroy();
	}
}
...
function createShip(x, y, sprite) {

	var Ship = game.add.sprite(x, y, sprite);
...
function update() {
	...
	if (ShiftButton.isDown && cursors.left.isDown) {
		playerShip.body.velocity.x = -800;
	}
